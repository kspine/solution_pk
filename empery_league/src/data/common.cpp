...
namespace Data {
	Poseidon::CsvParser sync_load_data(const char *file){
		...
		LOG_EMPERY_LEAGUE_INFO("Loading csv file: path = ", path);
		csv.load(path.c_str());
		return csv;
	}
	Poseidon::JsonArray encode_csv_as_json(const Poseidon::CsvParser &csv, const char *primary_key){
		...
		if(!data.empty()){
			...
			for(auto it = data.front().begin(); it != data.front().end(); ++it){
				if(it->first == primary_key){
					if(primary_key_column != -1){
						LOG_EMPERY_LEAGUE_ERROR("Duplicate primary key column: primary_key = ", primary_key);
			...
			if(primary_key_column == -1){
				LOG_EMPERY_LEAGUE_ERROR("Primary key column not found: primary_key = ", primary_key);
