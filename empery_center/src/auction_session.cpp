...
boost::shared_ptr<const ServletCallback> AuctionHttpSession::create_servlet(const std::string &uri, ServletCallback callback){
	...
	if(!weak.expired()){
		LOG_EMPERY_CENTER_ERROR("Duplicate auction HTTP servlet: uri = ", uri);
		DEBUG_THROW(Exception, sslit("Duplicate auction HTTP servlet"));
	}
...
boost::shared_ptr<Poseidon::Http::UpgradedSessionBase> AuctionHttpSession::on_low_level_request(
...
void AuctionHttpSession::on_sync_request(Poseidon::Http::RequestHeaders request_headers, Poseidon::StreamBuffer /* entity */){
	...
	if((uri.size() < m_prefix.size()) || (uri.compare(0, m_prefix.size(), m_prefix) != 0)){
		LOG_EMPERY_CENTER_WARNING("Inacceptable auction HTTP request: uri = ", uri, ", prefix = ", m_prefix);
