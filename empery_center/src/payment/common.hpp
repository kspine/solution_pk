#ifndef EMPERY_CENTER_PAYMENT_COMMON_HPP_
#define EMPERY_CENTER_PAYMENT_COMMON_HPP_
...
/*
PAYMENT_SERVLET(请求 URI, 返回 JSON 形参名, 会话形参名, GET 参数){
	// 处理逻辑
	return Response(code) <<msg;
}
*/
#define PAYMENT_SERVLET(uri_, root_arg_, session_arg_, params_arg_)	\
	namespace {	\
		namespace Impl_ {	\
			...
			{	\
				PROFILE_ME;	\
				LOG_EMPERY_CENTER_DEBUG("Payment servlet request: uri = ", uri_);	\
				return TOKEN_CAT3(PaymentServlet, __LINE__, Proc_) (root_, session_, ::std::move(params_));	\
			}	\
		}	\
	}	\
...
#define PAYMENT_THROW(code_)              DEBUG_THROW(::Poseidon::Http::Exception, (code_))
