...
MODULE_RAII_PRIORITY(handles, 9000){
	...
	auto port = get_config<unsigned>    ("player_websocket_server_port", 13211);
	...
	auto path = get_config<std::string> ("player_websocket_server_path", "/empery");

	const Poseidon::IpPort bind_addr(SharedNts(bind), port);
	LOG_EMPERY_CENTER_INFO("Creating WebSocket player server on ", bind_addr);
