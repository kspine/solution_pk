...
MODULE_RAII_PRIORITY(handles, 9000){
	...
	auto port = getConfig<unsigned>    ("player_websocket_server_port", 13211);
	...
	auto path = getConfig<std::string> ("player_websocket_server_path", "/empery");

	const Poseidon::IpPort bindAddr(SharedNts(bind), port);
	LOG_EMPERY_CENTER_INFO("Creating WebSocket player server on ", bindAddr);
