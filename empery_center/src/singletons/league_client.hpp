#ifndef EMPERY_CENTER_LEAGUE_CLIENT_HPP_
#define EMPERY_CENTER_LEAGUE_CLIENT_HPP_
...
class LeagueClient : public Poseidon::Cbpp::Client {
public:
	using Result = std::pair<Poseidon::Cbpp::StatusCode, std::string>;

	using ServletCallback = boost::function<
		Result (const boost::shared_ptr<LeagueClient> &, Poseidon::StreamBuffer)>;
...
public:
	...
	static boost::shared_ptr<LeagueClient> get();
	static boost::shared_ptr<LeagueClient> require();
...
public:
	LeagueClient(const Poseidon::SockAddr &sock_addr, bool use_ssl, std::uint64_t keep_alive_interval);
	~LeagueClient();
