...
{
	LOG_EMPERY_CENTER_INFO("league session constructor: this = ", (void *)this);
}
LeagueSession::~LeagueSession(){
	LOG_EMPERY_CENTER_INFO("league session destructor: this = ", (void *)this);
}

void LeagueSession::on_connect(){
	PROFILE_ME;
	LOG_EMPERY_CENTER_INFO("league session connected: remote = ", get_remote_info());
...
void LeagueSession::on_close(int err_code) noexcept {
	PROFILE_ME;
	LOG_EMPERY_CENTER_INFO("league session closed: err_code = ", err_code);
...
bool LeagueSession::on_low_level_data_message_end(std::uint64_t payload_size){
	PROFILE_ME;
	LOG_EMPERY_CENTER_TRACE("Received data message from league client: remote = ", get_remote_info(),
...
void LeagueSession::on_sync_control_message(Poseidon::Cbpp::ControlCode control_code, std::int64_t vint_param, std::string string_param){
	PROFILE_ME;
	LOG_EMPERY_CENTER_TRACE("Control message from league client: control_code = ", control_code,
