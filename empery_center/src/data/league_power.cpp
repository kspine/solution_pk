    ...
    namespace
    {
        ...
        MODULE_RAII_PRIORITY(handles, 900)
        {
            ...
            while(csv.fetch_row())
            {
                ...
                {
                    LOG_EMPERY_CENTER_ERROR("Duplicate union_power config: power_level = ", elem.leaguePower_level);
    ...
    namespace Data
    {
        ...
        const boost::shared_ptr<const LeaguePower> LeaguePower::get(std::uint64_t leaguePower_id)
        {
           ...
           const auto it = leaguePower_container->find(leaguePower_id);
            ...
            {
                LOG_EMPERY_CENTER_WARNING("corps_power config not found: slot = ", leaguePower_id);
                return { };
             }
