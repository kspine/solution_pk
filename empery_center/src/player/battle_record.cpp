...
PLAYER_SERVLET(Msg::CS_BattleRecordGetPagedCrateRecords, account, session, req){
	const auto crate_record_box = BattleRecordBoxMap::require_crate(account->get_account_uuid());
	crate_record_box->pump_status();

	std::vector<CrateRecordBox::RecordInfo> records;
	crate_record_box->get_all(records);
	...
	Msg::SC_BattleRecordPagedCrateRecords msg;
	msg.total_count = records.size();
	msg.begin = req.begin;
	msg.records.reserve(static_cast<std::size_t>(rend - rbegin));
	for(auto it = rbegin; it != rend; ++it){
		...
		record.second_coord_x         = it->second_coord.x();
		record.second_coord_y         = it->second_coord.y();
		record.resource_id            = it->resource_id.get();
		record.resource_harvested     = it->resource_harvested;
		record.resource_gained        = it->resource_gained;
		record.resource_remaining     = it->resource_remaining;
	}
	session->send(msg);
