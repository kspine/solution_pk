...
namespace {
	class LogServer : public Poseidon::TcpServerBase {
	public:
		LogServer(const Poseidon::IpPort &bind_addr, const std::string &cert, const std::string &private_key)
...
MODULE_RAII_PRIORITY(handles, 9000){
	...
	auto port = get_config<unsigned>    ("log_cbpp_server_port", 13224);
	...
	const auto server = boost::make_shared<LogServer>(bind_addr, cert, pkey);
