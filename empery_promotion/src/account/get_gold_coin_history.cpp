...
ACCOUNT_SERVLET("getGoldCoinHistory", session, params){
	...
	std::vector<boost::shared_ptr<MySql::Promotion_GoldCoinHistory>> objs;
	...
	oss <<"FROM `Promotion_GoldCoinHistory` WHERE 1=1 ";
	if(!time_begin.empty()){
		char str[256];
	...
	if(brief_mode.empty()){
		...
		auto promise = Poseidon::MySqlDaemon::enqueue_for_batch_loading(
			[&](const boost::shared_ptr<Poseidon::MySql::Connection> &conn){
				auto obj = boost::make_shared<MySql::Promotion_GoldCoinHistory>();
				obj->fetch(conn);
				objs.emplace_back(std::move(obj));
			}, "Promotion_GoldCoinHistory", oss.str());
