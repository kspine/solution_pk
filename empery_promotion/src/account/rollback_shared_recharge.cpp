...
ACCOUNT_SERVLET("rollbackSharedRecharge", session, params){
	...
	if(recharge_info.state != SharedRechargeMap::ST_FORMED){
		ret[sslit("errorCode")] = (int)Msg::ERR_SHARED_RECHARGE_NOT_FORMED;
		ret[sslit("errorMessage")] = "No formed shared recharge found";
		return ret;
	}

	SharedRechargeMap::rollback(account_id, recharge_to_account_id);
