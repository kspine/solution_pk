...
ACCOUNT_SERVLET("getIncomeBalanceHistory", session, params){
	...
	if(brief_mode.empty()){
		...
		oss <<"SUM(`income_balance`) AS `sum`, COUNT(*) AS `rows` ";
	}
	...
	if(brief_mode.empty()){
		...
		auto promise = Poseidon::MySqlDaemon::enqueue_for_batch_loading(
			[&](const boost::shared_ptr<Poseidon::MySql::Connection> &conn){
				auto obj = boost::make_shared<MySql::Promotion_IncomeBalanceHistory>();
				obj->fetch(conn);
				objs.emplace_back(std::move(obj));
			}, "Promotion_IncomeBalanceHistory", oss.str());
		...
		for(auto it = objs.begin(); it != objs.end(); ++it){
			...
			elem[sslit("incomeBalance")] = obj->get_income_balance();
