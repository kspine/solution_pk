...
namespace {
	...
	public:
		AdminHttpServer(const Poseidon::IpPort &bind_addr, const std::string &cert, const std::string &private_key,
...
MODULE_RAII_PRIORITY(handles, 9000){
	...
	auto port = get_config<unsigned>("admin_http_server_port", 10311);
	...
	auto path = get_config<std::string>("admin_http_server_path", "/empery_promotion/admin");

	const Poseidon::IpPort bind_addr(SharedNts(bind), port);
	LOG_EMPERY_PROMOTION_INFO("Creating admin HTTP server on ", bind_addr, path);
	const auto server = boost::make_shared<AdminHttpServer>(bind_addr, cert, pkey, std::move(auth), std::move(path));
