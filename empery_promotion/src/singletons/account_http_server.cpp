...
MODULE_RAII_PRIORITY(handles, 9000){
	...
	auto path = getConfig<std::string>("account_http_server_path", "/empery_promotion/account");

	const Poseidon::IpPort bindAddr(SharedNts(bind), port);
	LOG_EMPERY_PROMOTION_INFO("Creating account HTTP server on ", bindAddr, path);
